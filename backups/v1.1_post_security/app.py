...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        print(f"\n⚠️  Admin account created with password from .env file")
...
def add_trip():
    ...
    # Validate date range
    valid, error = validate_date_range(entry_date, exit_date)
    if not valid:
        flash(f'Error: {error}')
        return redirect(url_for('employee_detail', employee_id=employee_id))
    ...
    try:
        ...
        conn.execute('INSERT INTO trips (employee_id, country, entry_date, exit_date) VALUES (?, ?, ?, ?)', 
                    (employee_id, country_code, entry_date.strftime('%Y-%m-%d'), exit_date.strftime('%Y-%m-%d')))
        conn.commit()
        conn.close()
        ...
        flash(f'Error adding trip: {str(e)}')
