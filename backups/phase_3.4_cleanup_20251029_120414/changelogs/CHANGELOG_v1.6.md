...
## Phase 3.4 — Grid and Alignment Refinement (Maavsi Calendar)
- Rebuilt the calendar shell with a single CSS grid, sticky employee sidebar, and equal-width day columns for reliable alignment across zoom levels.
- Introduced refined TripLayer rendering to clamp trips inside the visible range, prevent duplicate overlays, and add `data-*` hooks for upcoming interactivity.
- Standardised typography, zebra striping, weekend shading, and soft row-hover highlights to deliver a balanced, professional layout.
- Enhanced responsiveness with automatic horizontal scrolling below 1000px and ensured layout parity with Phase 3.3 print/export styling.
