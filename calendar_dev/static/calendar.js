...
(function (global) {
    ...
    CalendarController.prototype.calculateResizeTarget = function calculateResizeTarget(session, deltaDays) {
        ...
        if (session.edge === 'start') {
            let proposed = addDays(session.baseStart, deltaDays);
            if (!proposed) {
                return null;
            }
            if (session.rangeStart && proposed < session.rangeStart) {
                proposed = session.rangeStart;
            }
            if (proposed > session.baseEnd) {
                proposed = session.baseEnd;
            }
            return { start: proposed, end: session.baseEnd };
        }
